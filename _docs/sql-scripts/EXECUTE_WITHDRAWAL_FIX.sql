...
-- 6. Atualizar configurações existentes se não tiverem os novos campos
UPDATE payment_settings 
SET 
  min_withdrawal_amount = COALESCE(min_withdrawal_amount, 10.00),
  max_withdrawal_amount = COALESCE(max_withdrawal_amount, 10000.00),
  withdrawal_fee_percent = COALESCE(withdrawal_fee_percent, 0.00),
  withdrawal_fee_fixed = COALESCE(withdrawal_fee_fixed, 0.00)
WHERE min_withdrawal_amount IS NULL OR max_withdrawal_amount IS NULL;
